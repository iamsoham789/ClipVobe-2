...
// Add console log to verify API key (remove in production)
...
const VideoScriptGenerator: React.FC<VideoScriptGeneratorProps> = ({
  ...
  console.log('VideoScriptGenerator initializing');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY}`, // Updated model name
        {
          method: 'POST',
          headers: {
...
${generatedText}`;
  ...
  try {
    console.log('VideoScriptGenerator rendering UI');
    return (
